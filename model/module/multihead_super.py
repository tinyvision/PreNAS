...
from .scaling_super import ScalingSuper
...
class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., normalization = False, relative_position = False,
                 num_patches = None, max_relative_position=14, scale=False, change_qkv = False, choices=None, scale_attn=False):
        super().__init__()
        ...
        if change_qkv:
            ...
            if scale_attn:
                self.qk_scaling = ScalingSuper([n_head * 64 for n_head in self.choices['num_heads']])
                self.v_scaling = ScalingSuper([n_head * 64 for n_head in self.choices['num_heads']])
        else:
    ...
    def set_sample_config(self, sample_q_embed_dim=None, sample_num_heads=None, sample_in_embed_dim=None):
        ...
        if not self.change_qkv:
            ...
            if self.scale_attn:
                self.qk_scaling.set_sample_config(sample_q_embed_dim)
                self.v_scaling.set_sample_config(sample_q_embed_dim)
    ...
    def forward(self, x):
        ...
        with autocast(enabled=False):
            ...
            if self.scale_attn:
                q = self.qk_scaling(q.transpose(1, 2).reshape(B, N, -1)).reshape(B, N, self.sample_num_heads, -1).transpose(1, 2)
            ...
            if self.scale_attn:
                v = self.v_scaling(v.transpose(1, 2).reshape(B, N, -1)).reshape(B, N, self.sample_num_heads, -1).transpose(1, 2)
